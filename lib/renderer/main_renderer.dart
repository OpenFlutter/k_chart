...
class MainRenderer extends BaseChartRenderer<CandleEntity> {
  ...
  MainRenderer(Rect mainRect, double maxValue, double minValue, double topPadding, this.state, this.isLine, int fixedLength,
  ...
  List<InlineSpan> _createMATextSpan(CandleEntity data) {
    ...
    for (int i = 0; i < data.maValueList.length; i++) {
      if (data.maValueList[i] != 0) {
        var item = TextSpan(text: "MA${maDayList[i]}:${format(data.maValueList[i])}    ", style: getTextStyle(ChartColors.getMAColor(i)));
        result.add(item);
      }
    }
    return result;
